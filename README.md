...
- **РЕВОЛЮЦИЯ ИНТЕРФЕЙСА И ВИЗУАЛА**
  * **Полное управление из трея:** Сворачивание в системный трей с функциональным контекстным меню.
  * **Плавающая панель настроек:** Вызывается кликом по иконке шестеренки в заголовке, не перекрывая основной интерфейс.
...
- **УСКОРЕНИЕ TELEGRAM И КРИПТОГРАФИЯ**
  * **Нативный Telegram SOCKS5:** Встроенный сервер на чистом C++ (WinHTTP API) с нулевой нагрузкой на ЦП.
  * **Дешифрование AES-CTR (MTProto):** Аппаратное дешифрование ключей через Windows BCrypt для безошибочной маршрутизации.
...
- **СИСТЕМНОЕ ЯДРО**
  * **Умная персистентность:** Автоматическое сохранение конфигураций при закрытии или сворачивании.
  * **Реальный TCP пинг:** Асинхронный пинг без зависаний интерфейса.
...
4. Нажмите **Initialise Local Bypass**, теперь можно свернуть приложение и наслаждаться.
...
4. Программа установит системные службы. Теперь можно закрыть окно кнопкой 'X' — прокси работает автономно!
